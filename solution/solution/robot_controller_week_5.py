...
#from assessment import ZoneLocation
...
#ZONE_TOP_LEFT = self.zones[ZoneLocation.TOP_LEFT] = Zone(ZoneLocation.TOP_LEFT, 2.57, 2.5)
#ZONE_TOP_RIGHT = self.zones[ZoneLocation.TOP_RIGHT] = Zone(ZoneLocation.TOP_RIGHT, 2.57, -2.46)
#ZONE_BOTTOM_RIGHT = self.zones[ZoneLocation.BOTTOM_RIGHT] = Zone(ZoneLocation.BOTTOM_RIGHT, -3.42, -2.46)
#ZONE_BOTTOM_LEFT = self.zones[ZoneLocation.BOTTOM_LEFT] = Zone(ZoneLocation.BOTTOM_LEFT, -3.42, 2.5)
...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.pose = PoseStamped().pose # Current pose (position and orientation), relative to the odom reference frame
        self.previous_pose = PoseStamped().pose # Store a snapshot of the pose for comparison against future poses
        ...
        self.current_goal = 0
        self.potential_zones = []

        self.spawn = self.pose.position

        self.potential_zones.append(Point(x = 2.57, y = 2.5))
        self.potential_zones.append(Point(x = 2.57, y = -2.46))
        self.potential_zones.append(Point(x = -3.42, y =  2.5))
        self.potential_zones.append(Point(x = -3.42, y =  -2.46))
        ...
        self.item_subscriber = self.create_subscription(
            ...
            'items',
            self.item_callback,
            10, callback_group=client_callback_group
        )
    ...
    def control_loop(self):
        ...
        match self.state:
            ...
            case State.OFFLOADING:
                ...
                self.get_logger().info(f"Zone: {zone.zone}, Size: {zone.size}, X: {zone.x}, Y: {zone.y}")
            
                estimated_distance = self.get_closest_zone()
                self.get_logger().info(f'Estimated distance {estimated_distance}')
    ...
    def holding_item(self):
        for item_holder in self.item_holders.data:
            self.get_logger().info(f"Item holder: {item_holder.robot_id}")
            if item_holder.robot_id == self.robot_id:
                return True
        self.get_logger().info(f"Robot {self.robot_id} is not holding an item")

    def get_closest_zone(self):
        position = self.pose.position
        closest_zone = self.potential_zones[0]
        shortest_distance = -1
        for zone in self.potential_zones:
            distance = math.sqrt((position.x - zone.x) ** 2 + (position.y - zone.y) ** 2)
            if distance < shortest_distance or shortest_distance == -1:
                closest_zone = zone
                shortest_distance = distance
            
        self.get_logger().info(f"Closest Zone is {closest_zone}")
